...
export function PlaceSidebar({ place, open, onOpenChange, onReviewAdded }: PlaceSidebarProps) {
  ...
  const handleReviewSubmit = async () => {
    ...
    if (!place) return;
    if (reviewRating === 0) { toast({ title: "별점 필수", variant: "destructive" }); return; }
    if (reviewContent.length < 30) { toast({ title: "30자 이상 필수", variant: "destructive" }); return; }
    if (reviewImage) {
      const validTypes = ["image/jpeg", "image/jpg", "image/png", "image/heic", "image/heif"];
      if (!validTypes.some(type => reviewImage.type.includes(type))) {
        toast({ title: "지원하지 않는 파일 형식 (JPG, PNG, HEIC만 가능)", variant: "destructive" }); return;
      }
    }

    setIsSubmittingReview(true);
    const token = getToken();
    if (!token) { toast({ title: "로그인 필요", variant: "destructive" }); setIsSubmittingReview(false); return; }
    ...
    try {
      ...
      if (reviewImage) {
        ...
        if (!presignedResponse.ok) { const err = await presignedResponse.json().catch(()=>({})); throw new Error(err.message || 'Presigned URL 요청 실패'); }
        ...
        const uploadResponse = await fetch(presignedUrl, { method: 'PUT', headers: { 'Content-Type': reviewImage.type }, body: reviewImage });
      ...
      if (!reviewSubmitResponse.ok) { throw new Error(reviewSubmitResult.message || '리뷰 등록 실패'); }
      ...
      <Card className={`${open ? "block" : "hidden lg:block"} sticky top-6 h-[calc(100vh-120px)] overflow-y-auto`}>
        <CardHeader className="flex flex-row items-start justify-between">
          ...
          <div className="flex gap-2">
            <Button variant="ghost" size="icon" onClick={() => setBookmarked(!bookmarked)}> <Bookmark className={bookmarked ? "fill-primary" : ""} /> </Button>
            <Button variant="ghost" size="icon" className="lg:hidden" onClick={() => onOpenChange(false)}> <X /> </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          {/* 장소 대표 이미지 (place.image 필드가 Place 타입에 정의되어 있어야 함) */}
          <img src={place.image || "/placeholder.svg"} alt={place.name} className="h-48 w-full rounded-lg object-cover"/>
          <div>
            ...
            <div className="mt-2 flex items-center gap-1">
              {Array.from({ length: 5 }).map((_, i) => ( <Star key={i} className={`h-4 w-4 ${i < Math.floor(place.averageRating ?? 0) ? "fill-secondary text-secondary" : "text-muted"}`}/> ))}
              <span className="ml-1 text-sm">{place.averageRating?.toFixed(1) ?? 'N/A'}</span>
            </div>
          </div>
          <Button onClick={() => setShowReviewDialog(true)} className="w-full" disabled={isSubmittingReview}>
            {isSubmittingReview ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null} 리뷰 등록
          </Button>
          <div className="space-y-3">
            <h3 className="font-semibold">리뷰 ({isLoadingReviews ? '...' : apiReviews.length}개)</h3>
            {isLoadingReviews ? ( <div className="flex justify-center py-4"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div> )
             : apiReviews.length === 0 ? ( <p className="text-sm text-muted-foreground">아직 리뷰가 없습니다.</p> )
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>별점 (필수)</Label>
              <div className="flex gap-1"> {Array.from({ length: 5 }).map((_, i) => ( <button key={i} type="button" onClick={() => setReviewRating(i + 1)}> <Star className={`h-6 w-6 ${i < reviewRating ? "fill-secondary text-secondary" : "text-muted"}`}/> </button> ))} </div>
            </div>
            <div className="space-y-2">
              <Label>내용 (30자 이상 필수)</Label>
              <Textarea value={reviewContent} onChange={(e) => setReviewContent(e.target.value)} placeholder="리뷰 내용을 작성하세요" rows={5} />
              <p className="text-xs text-muted-foreground">{reviewContent.length}/30자</p>
            </div>
            <div className="space-y-2">
              <Label>이미지 (선택, JPG/PNG/HEIC)</Label>
              <Input type="file" accept="image/jpeg,image/jpg,image/png,image/heic,image/heif" onChange={(e) => setReviewImage(e.target.files?.[0] || null)} disabled={isSubmittingReview} />
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={resetReviewFormAndCloseDialog} className="flex-1" disabled={isSubmittingReview}>취소</Button>
              <Button onClick={handleReviewSubmit} className="flex-1" disabled={isSubmittingReview}>
